...
- **A. Decoupled Source and Hunt.** `C004` Source Registry holds per-instance auth and adapter binding; `C006` Hunt Registry holds named collections of `(source, selector)` entries. `C005` Source Adapter is instantiated against a registered source; `C008` Refresh Engine iterates a hunt's entries and asks the adapter per selector. Cost: two declaration verbs (`source`, `hunt`) instead of one combined `watch`; the bird-dogger learns the distinction up front; an item that appears in two hunts via the same source is one item under `C007`'s identity but two membership facts under `C016`.
...
The decoupling is also the structural backbone the rest of the engineering layer assumes: `C005` Source Adapter is source-type-agnostic and bound to a source, not a hunt; `C006` validates source references against `C004`; `C008` receives a hunt scope and asks `C005` per source.

## Consequences

- `C004` Source Registry and `C006` Hunt Registry stay separate components, each with their own CLI surface (`source` verbs and `hunt` verbs in `C001`). Two CRUD verb families to design and document.
...
- PRD003's "one bundle, two outcome-layer requirements" lands cleanly: `C002` declaration + `C003` secret + `C004` registry + `C005` adapter contract is one implementation that satisfies both `J001-O001-R007` and `J001-O002-R007`.
- Source availability (`J001-O001-R006`) is reported per source by `C005`, not per hunt-membership. `C008` carries the per-hunt source-availability snapshot into `C016` so it can be rendered alongside the affected items in any hunt that uses the affected source.
- Item identity (`C007`) is source-scoped, not hunt-scoped: the same Jira issue surfaced in two hunts is one item under `C007` and two membership rows under `C016`.
...
- Future source types (other trackers, doc systems, message platforms) plug in by implementing the `C005` Source Adapter contract; `C006` stays source-type-agnostic and selectors are source-type-specific shapes the adapter knows how to interpret. The Source Adapter contract shape itself is component-level ADR territory under `C005`.
