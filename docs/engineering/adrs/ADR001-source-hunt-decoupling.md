...
- **A. Decoupled Source and Hunt.** **Source Registry (C004)** holds per-instance auth and adapter binding; **Hunt Registry (C006)** holds named collections of `(source, selector)` entries. **Source Adapter (C005)** is instantiated against a registered source; **Refresh Engine (C008)** iterates a hunt's entries and asks the adapter per selector. Cost: two declaration verbs (`source`, `hunt`) instead of one combined `watch`; the bird-dogger learns the distinction up front; an item that appears in two hunts via the same source is one item under **Item Store (C007)**'s identity but two membership facts under **Coverage Memory (C016)**.
...
- **Item Store (C007)** identity is source-scoped, not hunt-scoped: the same Jira issue surfaced in two hunts is one item in **Item Store (C007)** and two membership rows in **Coverage Memory (C016)**.
