...
- **Refresh Engine (C008)** walks a hunt's selector references, resolves each via **Selector Registry (C017)**, and asks the bound **Source Adapter (C005)** to pull items per resolved triple. Source-side rate-limit pressure is unchanged from ADR001: one burst per refresh, bounded by the selectors in scope.
